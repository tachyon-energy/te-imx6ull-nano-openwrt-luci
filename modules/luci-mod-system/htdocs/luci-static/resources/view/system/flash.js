...
function findStorageSize(procmtd, procpart) {
	...
	procmtd.split(/\n/).forEach(function(ln) {
		var match = ln.match(/^mtd\d+: ([0-9a-f]+) [0-9a-f]+ "(.+)"$/),
		    size = match ? parseInt(match[1], 16) : 3;
...
return view.extend({
	...
	handleSysupgrade: function(storage_size, has_rootfs_data, ev) {
		return ui.uploadFile('/tmp/firmware.bin', ev.target.firstChild)
			.then(L.bind(function(btn, reply) {
			 ...
			 .then(L.bind(function(btn, reply) {
            return fs.exec('/sbin/update', [ '--test' , '/tmp/firmware.bin' ])
                    .then(function(res) { return [ reply, res ]; });
			}, this, ev.target))
			.then(L.bind(function(btn, res) {
				/* sysupgrade opts table  [0]:checkbox element [1]:check condition [2]:args to pass */
				var opts = {
				    keep : [ E('input', { type: 'checkbox' }), false, '-e' ],
				    force : [ E('input', { type: 'checkbox' }), true, '--force' ],
				    skip_orig : [ E('input', { type: 'checkbox' }), true, '-u' ],
				    backup_pkgs : [ E('input', { type: 'checkbox' }), true, '-b' ],
				    },
				    is_valid = true, //res[1].valid,
				    is_forceable = true, //res[1].forceable,
				    allow_backup = true, //res[1].allow_backup,
				    is_too_big = false, //(storage_size > 0 && res[0].size > storage_size),
				...
				if (res[1].code === 2) {
					body.push(E('pre', {}, E('label', { 'class': 'btn alert-message warning' }, [
						opts.force[0], ' ', _('Force upgrade'),
						E('br'), E('br'),
						res[1].stdout ? res[1].stdout : '',
						_('The uploaded firmware have oldest version. Select \'Force upgrade\' to flash the image. Use only if you are sure that the firmware is correct!')
					])));
					...
					body.push(E('pre', { 'class': 'alert-message danger' }, [
						res[1].stdout ? res[1].stdout : '',
						res[1].stdout ? E('br') : '',
                ...
                body.push(E('pre', { 'class': 'alert-message success' }, [
                    _(res[1].stdout)
                ]));
	...
	render: function(rpc_replies) {
		...
		o = s.option(form.SectionValue, 'actions', form.NamedSection, 'actions', 'actions', _('Flash new firmware image'),
			has_sysupgrade
				? _('Upload a Tachyon Energy-compatible image here to replace the running firmware.')
